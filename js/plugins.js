...
function toFixedT(value, numdecs) {
  ...
  switch (parseInt(numdecs)) {
    ...
    case 1:
      str = value.toString().match(/^\d+(?:\.\d{0,1})?/);
      break;
    case 2:
      str = value.toString().match(/^\d+(?:\.\d{0,2})?/);
      break;
    case 3:
      str = value.toString().match(/^\d+(?:\.\d{0,3})?/);
      break;
    case 4:
      str = value.toString().match(/^\d+(?:\.\d{0,4})?/);
      break;
    default:
      return value.toFixed(numdecs); // use default javascript routine
  }
...
(function ($) {
  ...
  var config = {
    ...
    onUpdate: function (tstamp, running, paused) {
      return true;
    }, // action to do on display new timestamp
  ...
  function run_chrono() {
    ...
    if (paused) {
      ...
      // chrono stopped; show data at least once
  ...
  function view_chrono(elapsed) {
    ...
    if (config.showMode == 0) {
      ...
      $(config.seconds_sel).html(
        "" + toFixedT(parseFloat(elapsed / 1000.0), digits) + extra
      );
    ...
    if (methods[args]) {
      ...
      return this.each(function () {
        ...
        if (config.auto) {
          $(this).html(
            '<span id="' +
              format_selector(config.days_sel) +
              '" data-days = "' +
              config.days +
              '">' +
              view_format(config.days) +
              '</span>:<span id="' +
              format_selector(config.hours_sel) +
              '" data-hours = "' +
              config.hours +
              '">' +
              view_format(config.hours) +
              '</span>:<span id="' +
              format_selector(config.minutes_sel) +
              '" data-minutes = "' +
              config.minutes +
              '">' +
              view_format(config.minutes) +
              '</span>:<span id="' +
              format_selector(config.seconds_sel) +
              '" data-seconds = "' +
              config.seconds +
              '">' +
              view_format(config.seconds) +
              "</span>"
          );
        }
